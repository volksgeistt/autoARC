...
def resize_terminal() -> None:
    try:
        if os.name == "nt":
            os.system(f"mode con cols={_TARGET_COLS} lines={_TARGET_ROWS}")
        else:
            print(f"\x1b[8;{_TARGET_ROWS};{_TARGET_COLS}t", end="", flush=True)
    except Exception:
        pass  
...
MENU_ITEMS = [
    ...
    ("10", "SysSentinel",         "CPU / RAM / GPU / Disk monitor + alerts"),
    ("11", "NetWatch",            "Live ping, packet loss, bandwidth usage"),
...
_LABEL_W = 24
_SEP2    = 3
_DESC_W  = _CONTENT_W - _KEY_W - _SEP1 - _LABEL_W - _SEP2  
...
def _menu_row(key: str, label: str, desc: str) -> str:
    if key == "0":
        ...
        lc = Fore.YELLOW  + Style.BRIGHT
    elif key in ("10", "11"):
        kc = Fore.MAGENTA + Style.BRIGHT
        lc = Fore.MAGENTA + Style.BRIGHT
    else:
        kc = Fore.CYAN    + Style.BRIGHT
        lc = Fore.WHITE   + Style.BRIGHT

    key_str  = f"[{key:>2}]"                       
    key_part = f"{kc}{key_str}{Style.RESET_ALL}"
    lbl_part = f"{lc}{label:<{_LABEL_W}}{Style.RESET_ALL}"
    d        = desc[:_DESC_W]
    dsc_part = f"{Fore.LIGHTBLACK_EX}{d:<{_DESC_W}}{Style.RESET_ALL}"

    return f"{key_part}{' ' * _SEP1}{lbl_part}{' ' * _SEP2}{dsc_part}"


def _boxed(row_content: str) -> str:
    vis    = len(_strip_ansi(row_content))
    pad    = max(0, _CONTENT_W - vis)
...
def _edge(left: str, fill: str, right: str) -> str:
    border = Fore.CYAN + Style.BRIGHT
    line   = f"{border}{left}{fill * (_BOX_W - 2)}{right}{Style.RESET_ALL}"
    return center(line)
...
def display_menu() -> None:
    ...
    hdr = (
        f"{Fore.CYAN}{Style.BRIGHT}{'#':<{_KEY_W}}"
        f"{' ' * _SEP1}"
        f"{Fore.WHITE}{Style.BRIGHT}{'FEATURE':<{_LABEL_W}}"
        f"{' ' * _SEP2}"
        f"{Fore.LIGHTBLACK_EX}{'DESCRIPTION':<{_DESC_W}}"
    )

    print(TOP)
    print(center(_boxed(hdr)))
    print(SEP)
